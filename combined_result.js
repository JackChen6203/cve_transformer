...
async function updatePromptsWithResults() {
    const [prompts] = await pool.query('SELECT id, prompt FROM prompts WHERE result IS NULL');

    for (const prompt of prompts) {
        const modifiedPrompt = checkAndReplace(prompt.prompt);
        try {
            ...
            await pool.query('UPDATE prompts SET result = ? WHERE id = ?', [result, prompt.id]);
